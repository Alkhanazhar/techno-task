...
const TimelineJourney = () => {
  ...
  useEffect(() => {
    ...
    const pathLength = pathRef.current.getTotalLength();
    const totalSentences = data.reduce((acc, step) => acc + step.content.length, 0);
  ...
  return (
    <motion.div ref={containerRef} className="relative h-[250vh] py-10 bg-slate-200">
      {/* Sticky timeline */}
    <div className="absolute inset-0 z-0">
            <img
              src="/bgPattern.png"
              alt="Background Pattern"
              className="w-screen h-full object-cover opacity-15 rotate-90 !z-0"
            />
          </div>
      ...
      >
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="text-center md:-mt-8">
            <h2 className="text-3xl font-bold text-gray-800 mb-b pb-16 pt-10">Our Journey</h2>
          </div>
          ...
          <div ref={containerRef} className="relative py-20">
            ...
            <div className="relative h-[400px]">
              ...
              >
                <defs>
                  <pattern id="zigzagDots" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse">
                    <circle cx="5" cy="5" r="1.5" fill="#d1d5db" />
                  </pattern>
                  <pattern id="activeZigzagDots" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse">
              ...
              {data.map((step, index) => {
                ...
                return (
                  ...
                  >
                    {/* Step indicator dot */}
                    <motion.div
                      className={`w-4 h-4 rounded-full border-2 z-30 ${isPast ? 'bg-purple-500 border-purple-500' :
                    ...
                    <motion.div
                      ...
                      `}
                      initial={{ opacity: 0, y: index % 2 === 0 ? 20 : -20, scale: 0.8 }}
                      animate={{
                        opacity: isVisible ? 1 : 0,
                        y: isVisible ? 0 : (index % 2 === 0 ? 20 : -20),
                        scale: isVisible ? 1 : 0.8,
                        filter: isPast ? 'blur(0px)' : isActive ? 'blur(0px)' : 'blur(6px)',  // <--- add this
                      }}
                      transition={{
                        duration: 0.2,
                        ease: "easeOut"
    ...
    ${isActive ? ' bg-gradient-to-r from-[#C068D1] to-[#3224AF] border-purple-600  scale-100' :
                            isPast ? 'border-green-400 bg-green-50' :
                              'border-gray-300 bg-gray-50'}
  `}
                        animate={{
                          scale: isActive ? 1.05 : 1,
                          borderColor: isActive ? '#9333ea' : isPast ? '#C068D1' : '#d1d5db'
                        }}
                        transition={{ duration: 0.15 }}
                      >
                        <div className={`text-xs font-semibold mb-2 ${isActive ? 'text-purple-100' :
                          isPast ? 'text-[#9333ea]' :
                            'text-gray-600'
                          }`}>
                          {step.stepLabel}
                        </div>
                        <p className={`text-xs leading-relaxed transition-1 ${isActive ? 'text-white' : 'text-gray-800'}`}>
